...
export default function EditMenuScreen() {
  ...
  const [originalRestaurantId, setOriginalRestaurantId] = useState<number>(0);
  ...
  const loadMenuData = async () => {
    ...
    try {
      ...
      setImage(menu.image);
      // Store the restaurant ID (in real app, this would come from the menu response)
      // For now, we'll use 1 as placeholder
      setOriginalRestaurantId(1);
    } catch (error) {
  ...
  const handleUpdateMenu = async () => {
    ...
    try {
      ...
      await updateMenu(parseInt(menuId), menuRequest);
